...
{
    public class PeriodicalRotateNotes : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!(Main.npc[(int)projectile.ai[0]].type == ModContent.NPCType<NecrodonDDDFight>() && Main.npc[(int)projectile.ai[0]].active))
            {
                projectile.Kill();
                return;
            }
        ...
        {
            if(counter < (int)Main.npc[(int)projectile.ai[0]].ai[1])
            {
                counter++;
                if(counter % 2 == 0)
                {
