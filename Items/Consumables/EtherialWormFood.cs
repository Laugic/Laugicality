...
{
	public class EtherialWormFood : ModItem
    {
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            if (NPC.CountNPCS(NPCID.EaterofWorldsHead) < 1)
                NPC.NewNPC((int)player.position.X, (int)player.position.Y - 480, NPCID.EaterofWorldsHead);
            return false;
        }
        ...
        {
            var modPlayer = player.GetModPlayer<LaugicalityPlayer>(mod);
            return (player.ZoneCorrupt && modPlayer.etherial);
        }
