        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            while (power > 0)
            {
                ...
                if (Main.myPlayer == projectile.owner)
                    Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, mag * (float)Math.Cos(theta), mag * (float)Math.Sin(theta), ModContent.ProjectileType<FrostballProjectile>(), projectile.damage, 3f, Main.myPlayer);
            }
