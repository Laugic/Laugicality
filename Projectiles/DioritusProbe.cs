...
namespace Laugicality.Projectiles
{
    ...
    public class DioritusProbe : HoverShooter
    {
        ...
        public override void SetDefaults()
        {
            ...
            inertia = 12f;
            shoot = mod.ProjectileType("DaggorShard");
            shootCool = 30f;
            shootSpeed = 18f;
        }
        ...
        public override void Behavior()
        {
            Player player = Main.player[projectile.owner];
            if (index == 0)
                index = player.ownedProjectileCounts[mod.ProjectileType("DioritusProbe")];
                ...
                if (target)
                {
                    ...
                    if (projectile.ai[1] == 0f)
                    {
                        ...
                        if (Main.myPlayer == projectile.owner)
                        {
                            ...
                            shootVel.Normalize();
                            shootVel *= shootSpeed;
                            int proj = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, 0, 6, shoot, projectile.damage, projectile.knockBack, Main.myPlayer, 0f, 0f);
