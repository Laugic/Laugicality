        ...
        public void Shoot(NPC npc)
        {
            if (counter <= 0)
            {
                ...
                if (Main.myPlayer == projectile.owner)
                    Projectile.NewProjectile(projectile.position, new Vector2(projectile.DirectionTo(npc.Center).X * 6, projectile.DirectionTo(npc.Center).Y * 6), ProjectileID.MagnetSphereBall, projectile.damage, 4, projectile.owner);
            }
            else
                counter--;
        }
