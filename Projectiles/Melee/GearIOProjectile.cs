        ...
        public override void PostAI()
        {
            ...
            if (reload <= 0)
            {
                ...
                if (Main.myPlayer == projectile.owner)
                    Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, (float)Math.Cos(theta) * mag, (float)Math.Sin(theta) * mag, ModContent.ProjectileType<GearIO2Projectile>(), projectile.damage, 3f, Main.myPlayer);
            }
        }
    }
}
